...
# Stage 5: With Data augmentation
if [ $stage -le 5 ]; then
  ...
  ### Monophone
    echo "Monophone training with augmented data"
	steps/train_mono.sh --nj "$nj" --cmd "$train_cmd" data/train_sp3 data/lang exp/mono
    echo "Monophone training done"
    (
    echo "Decoding the test set"
    utils/mkgraph.sh data/lang exp/mono exp/mono/graph
  ...
  ### Triphone
    echo "Triphone training for augemted data"
     steps/align_si.sh --nj $nj --cmd "$train_cmd" \
